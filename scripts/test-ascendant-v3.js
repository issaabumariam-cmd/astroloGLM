...
// Use getAllPlanets to initialize everything, then access internal sidereal time
function getRAMC(lng, lat, date) {
  // This initializes constant, processor, etc.
  const r = e.getAllPlanets(date, lng, lat, 0);
  
  // After getAllPlanets, the constant module has the date set up
  // We can access the internal sidereal time via the constant module
  ...
  // sidereal.calc returns local apparent sidereal time in seconds
  const sidSeconds = sidereal.calc(constant.date, constant.tlong);
  let ramc = (sidSeconds / 240) % 360; // 240 seconds per degree
...
function calcAsc(ramc, lat) {
  const constant = require('../node_modules/ephemeris/src/astronomy/moshier/constant');
  // constant.eps is set after getAllPlanets — it's the obliquity in radians
  const epsDeg = constant.eps * constant.RTD; // convert radians to degrees
  const oblRad = epsDeg * Math.PI / 180;
  ...
  asc = ((asc % 360) + 360) % 360;
  return { asc, flip: (asc + 180) % 360, epsDeg };
}
...
console.log('Name          MC           RAMC      eps     raw              flip             expected');
console.log('-----------   ----------   -------  ------   --------------   --------------   -----------');
...
testCases.forEach(tc => {
  ...
  console.log(
    tc.name.padEnd(13) +
    mcSign.padEnd(13) +
    ramc.toFixed(2).padStart(8) + '  ' +
    result.epsDeg.toFixed(4).padStart(7) + '  ' +
